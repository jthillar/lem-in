...
/*   Updated: 2017/10/06 17:51:13 by jthillar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	end_line(char *line, t_startend *se)
{
	if (se->com == 0 && se->ants > 0 && ft_parse_sel(line, se) == 2 && se->w_roomentry < 2
		&& ft_p_room(line) == -1)
	{
		ft_memdel((void **)&line);
		return ;
...
int		main(void)
{
	...
	while (get_next_line(0, &line) == 1)
	{
		...
		if (COM && ANT && PARSE && ft_p_room(line) == -1 && se.w_roomentry > 2)
		{
